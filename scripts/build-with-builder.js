...
/**
 * Simplified build script for Zhanlu Work
...
try {
  ...
  if (process.platform === 'win32' && builderArgs.includes('--win')) {
    ...
    if (!cleaned) {
      const aionRunning = isProcessRunningWindows('ZhanluWork.exe');
      const electronRunning = isProcessRunningWindows('electron.exe');
      if (aionRunning || electronRunning) {
        console.log('⚠️  Detected running Zhanlu Work/Electron process. Attempting to close...');
        killWindowsProcesses(['ZhanluWork.exe', 'electron.exe']);
        cleaned = tryRemoveDir(winUnpackedDir);
        if (!cleaned) {
          console.log('⚠️  Directory still locked. Please close any running Zhanlu Work/Electron processes and retry.');
        }
      }
    }
  }
  ...
  try {
    ...
    const winExePath = path.join(outDir, 'win-unpacked', 'ZhanluWork.exe');
    ...
    console.log('⚠️  Windows local build failed after ZhanluWork.exe was produced.');
    if (firstError) {
