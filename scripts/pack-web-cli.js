...
const tarballName = `zhanlu-work-web-${version}-${normalizedPlatform}-${normalizedArch}.tar.gz`;
...
const tarballContentDir = path.join(stagingDir, 'zhanlu-work-web');
...
const executableName = platform === 'win32' ? 'zhanlu-work-web.exe' : 'zhanlu-work-web';
...
// `zhanlu-work-web version` match the tarball filename.
...
execSync(`tar -czf ${path.basename(tarballPath)} -C ${stagingDir} zhanlu-work-web`, {
