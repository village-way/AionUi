#!/usr/bin/env bash
# ============================================================================
# Zhanlu Work WebUI — One-Click Installation Script
# ============================================================================
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/Ecloud/ZhanluWork/main/scripts/install-web.sh | bash
#   # Or specify version:
#   VERSION=1.0.0 bash install-web.sh
#   # Or install to custom directory:
#   INSTALL_DIR=/opt/zhanlu-work-web bash install-web.sh
# ============================================================================
...
INSTALL_DIR="${INSTALL_DIR:-${HOME}/.local/share/zhanlu-work-web}"
BIN_DIR="${BIN_DIR:-${HOME}/.local/bin}"
MIRROR="${MIRROR:-https://github.com/Ecloud/ZhanluWork/releases/download}"
...
banner() {
    echo -e "${CYAN}${BOLD}"
    echo "  ╔══════════════════════════════════════════════╗"
    echo "  ║   Zhanlu Work WebUI Installer (No Electron)  ║"
    echo "  ╚══════════════════════════════════════════════╝"
    echo -e "${NC}"
}
...
Options:
  ...
  --install-dir <path>      Specify installation directory (default: ~/.local/share/zhanlu-work-web)
...
detect_platform_arch() {
    ...
    TARBALL_NAME="zhanlu-work-web-${VERSION}-${PLATFORM}-${ARCH}.tar.gz"
...
download_tarball() {
    ...
    # Build download URL
    # MIRROR formats:
    #   - GitHub: https://github.com/Ecloud/ZhanluWork/releases/download
...
extract_tarball() {
    ...
    # Extract tarball
    # Tarball root directory is zhanlu-work-web/, rename after extraction to INSTALL_DIR
    ...
    if [[ -d "${extract_temp}/zhanlu-work-web" ]]; then
        mv "${extract_temp}/zhanlu-work-web" "$INSTALL_DIR"
    else
        die "Tarball structure is invalid (missing zhanlu-work-web/ directory)"
    fi

    success "Extracted to $INSTALL_DIR"

    # Set executable permission on the bun-compiled standalone binary
    chmod +x "${INSTALL_DIR}/zhanlu-work-web" 2>/dev/null || true
    ...
    # Verify installation
    if [[ ! -x "${INSTALL_DIR}/zhanlu-work-web" ]]; then
        die "Installation failed: ${INSTALL_DIR}/zhanlu-work-web not found or not executable"
    fi
...
create_symlink() {
    local symlink_path="${BIN_DIR}/zhanlu-work-web"
    local target_path="${INSTALL_DIR}/zhanlu-work-web"
...
update_shell_profile() {
    ...
    # Add to profile
    echo "" >> "$profile_file"
    echo "# Added by zhanlu-work-web installer" >> "$profile_file"
...
print_summary() {
    ...
    echo -e "${GREEN}${BOLD}  🎉 Zhanlu Work WebUI v${VERSION} Installed!${NC}"
    echo -e "${GREEN}${BOLD}══════════════════════════════════════════════════${NC}"
    echo ""
    echo -e "  ${BOLD}📍 Installation directory:${NC}  ${INSTALL_DIR}"
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo -e "  ${BOLD}📍 Symlink:${NC}                ${BIN_DIR}/zhanlu-work-web"
    fi
    echo ""
    echo -e "  ${BOLD}🚀 Usage:${NC}"
    echo ""
    if [[ "$CREATE_SYMLINK" == "1" && ":$PATH:" == *":${BIN_DIR}:"* ]]; then
        echo "    # Start Zhanlu Work WebUI"
        echo "    zhanlu-work-web start"
        echo ""
        echo "    # Check version"
        echo "    zhanlu-work-web version"
    else
        echo "    # Start Zhanlu Work WebUI (using full path)"
        echo "    ${INSTALL_DIR}/zhanlu-work-web start"
        echo ""
        echo "    # Or add symlink to PATH:"
        if [[ "$CREATE_SYMLINK" == "1" ]]; then
            echo "    export PATH=\"${BIN_DIR}:\$PATH\""
        else
            echo "    ln -s ${INSTALL_DIR}/zhanlu-work-web ~/.local/bin/zhanlu-work-web"
            echo "    export PATH=\"~/.local/bin:\$PATH\""
        fi
    fi
    echo ""
    echo -e "  ${BOLD}📖 Documentation:${NC}  https://github.com/Ecloud/ZhanluWork"
    echo -e "  ${BOLD}🐛 Report issues:${NC}  https://github.com/Ecloud/ZhanluWork/issues"
    echo ""
    ...
    if [[ "$CREATE_SYMLINK" == "1" ]]; then
        echo ""
        echo "    # Remove symlink"
        echo "    rm ${BIN_DIR}/zhanlu-work-web"
    fi
