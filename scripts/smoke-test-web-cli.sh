...
if [ ! -d "$TEMP_DIR/zhanlu-work-web" ]; then
  echo "❌ Missing zhanlu-work-web directory"
  exit 1
fi

cd "$TEMP_DIR/zhanlu-work-web"

# New layout (bun compile standalone binary):
#   zhanlu-work-web/
#   ├── zhanlu-work-web           ← single compiled executable (no bin/, no dist/, no node_modules)
...
if [ ! -x "zhanlu-work-web" ]; then
  echo "❌ zhanlu-work-web is not executable"
  exit 1
fi
echo "✓ zhanlu-work-web is executable"

# 4. Test version command
echo ""
echo "4. Testing version command..."
VERSION=$(./zhanlu-work-web version)
if [ -z "$VERSION" ]; then
...
HTTP_PORT=25899
DATA_DIR="$(mktemp -d)/zhanlu-work-web-data"
...
./zhanlu-work-web start --port "$HTTP_PORT" --data-dir "$DATA_DIR" > /tmp/zhanlu-work-web.log 2>&1 &
SERVER_PID=$!
...
for i in $(seq 1 30); do
  if curl -sf "http://127.0.0.1:${HTTP_PORT}/" > /tmp/zhanlu-work-web.html 2>/dev/null; then
    break
  fi
  sleep 1
done

if [ ! -s /tmp/zhanlu-work-web.html ]; then
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP probe failed — no response body. Server log:"
  cat /tmp/zhanlu-work-web.log
  exit 1
fi

# Look for the SPA shell signature — <html + <div id="root" or similar marker
if grep -q '<html' /tmp/zhanlu-work-web.html && grep -qE '<(div id="root"|script)' /tmp/zhanlu-work-web.html; then
  echo "✓ HTTP root returns SPA index ($(wc -c < /tmp/zhanlu-work-web.html) bytes)"
else
  kill "$SERVER_PID" 2>/dev/null || true
  wait "$SERVER_PID" 2>/dev/null || true
  echo "❌ HTTP root response does not look like SPA index:"
  head -20 /tmp/zhanlu-work-web.html
  echo "---server log---"
  cat /tmp/zhanlu-work-web.log
  exit 1
fi
...
if grep -q 'Backend binary not found' /tmp/zhanlu-work-web.log; then
  ...
  for i in $(seq 1 20); do
    PASSWORD=$(grep -oE 'Generated initial admin password: [^ ]+' /tmp/zhanlu-work-web.log | head -1 | sed 's/^Generated initial admin password: //')
    if [ -n "$PASSWORD" ]; then
      break
    fi
    sleep 1
