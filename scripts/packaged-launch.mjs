...
function resolvePackagedApp(projectRoot) {
  ...
  if (process.platform === 'win32') {
    for (const dir of ['win-unpacked', 'win-x64-unpacked', 'win-arm64-unpacked']) {
      const exe = path.join(outDir, dir, 'ZhanluWork.exe');
    ...
    for (const dir of ['mac-arm64', 'mac-x64', 'mac', 'mac-universal']) {
      ...
      const exe = path.join(macDir, appBundle, 'Contents', 'MacOS', 'ZhanluWork');
    ...
    for (const dir of ['linux-unpacked', 'linux-x64-unpacked', 'linux-arm64-unpacked']) {
      ...
      for (const name of ['ZhanluWork', 'zhanlu-work']) {
...
async function main() {
  ...
  if (shouldClean) {
    await killProcessByName('ZhanluWork.exe');
    await killProcessByName('ZhanluWork');
