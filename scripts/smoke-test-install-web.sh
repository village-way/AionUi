...
export INSTALL_DIR="/tmp/zhanlu-work-web-smoke-test"
...
if [[ ! -x "${INSTALL_DIR}/zhanlu-work-web" ]]; then
    echo "❌ CLI executable not found or not executable: ${INSTALL_DIR}/zhanlu-work-web"
    exit 1
fi
echo "✓ CLI executable exists"

if [[ ! -L "${BIN_DIR}/zhanlu-work-web" ]]; then
    echo "❌ Symlink not found: ${BIN_DIR}/zhanlu-work-web"
    exit 1
fi
...
VERSION_OUTPUT=$(zhanlu-work-web version 2>&1 || echo "")
