...
const makeGitHubReleaseResponse = () => [
  {
    ...
    assets: [
      ...
      {
        name: 'ZhanluWork-1.9.22-linux-amd64.deb',
        browser_download_url:
          'https://github.com/Ecloud/ZhanluWork/releases/download/v1.9.22/ZhanluWork-1.9.22-linux-amd64.deb',
        size: 789,
      },
    ],
  },
...
describe('updateBridge CDN URL rewriting', () => {
  ...
  it('rewrites asset.url to the CDN path and keeps GitHub URL in fallbackUrl', async () => {
    ...
    try {
      ...
      const linuxAsset = assets.find((a: { name: string }) => a.name === 'ZhanluWork-1.9.22-linux-amd64.deb');
      expect(linuxAsset?.url).toBe('https://updates.zhanlu.work/releases/1.9.22/ZhanluWork-1.9.22-linux-amd64.deb');
    } finally {
      vi.unstubAllGlobals();
    }
  });
