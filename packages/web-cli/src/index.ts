...
// tarball layout:
//   zhanlu-work-web/
//   ├── zhanlu-work-web              ← bun-compiled standalone binary (process.execPath)
...
function resolveCliRoot(): string {
  // Heuristic: if the executable path ends in the release CLI name, treat it
  // as the packaged single-file binary and return its directory. The legacy
  // executable names are accepted only so older local symlinks keep working.
  const exe = process.execPath;
  const exeName = path.basename(exe).toLowerCase();
  if (['zhanlu-work-web', 'zhanlu-work-web.exe', 'aionui-web', 'aionui-web.exe'].includes(exeName)) {
...
const isPackaged = (() => {
  ...
  return ['zhanlu-work-web', 'zhanlu-work-web.exe', 'aionui-web', 'aionui-web.exe'].includes(exeName);
})();

const BACKEND_BINARY = process.platform === 'win32' ? 'aioncore.exe' : 'aioncore';
const DEFAULT_PORT = 25808;
const RESET_COMMAND = isPackaged ? 'zhanlu-work-web resetpass' : 'bun run resetpass';
...
function resolveBackendBinary(flags: Map<string, string | true>): string {
  ...
  const envOverride = process.env.ZHANLU_WORK_BACKEND_BIN ?? process.env.AIONUI_BACKEND_BIN;
...
function resolveDataDir(flags: Map<string, string | true>): string {
  ...
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.ZHANLU_WORK_DATA_DIR ?? process.env.AIONUI_DATA_DIR;
  if (envOverride) return path.resolve(envOverride);
  return path.join(os.homedir(), '.zhanlu-work-web');
}

function resolveLogDir(flags: Map<string, string | true>, dataDir: string): string {
  const override = flags.get('log-dir');
  if (typeof override === 'string') return path.resolve(override);
  const envOverride = process.env.ZHANLU_WORK_LOG_DIR ?? process.env.AIONUI_LOG_DIR;
  if (envOverride) return path.resolve(envOverride);
...
function resolvePort(flags: Map<string, string | true>): number {
  ...
  const env = process.env.ZHANLU_WORK_PORT ?? process.env.AIONUI_PORT ?? process.env.PORT;
...
function resolveAllowRemote(flags: Map<string, string | true>): boolean {
  ...
  const env =
    process.env.ZHANLU_WORK_ALLOW_REMOTE ??
    process.env.ZHANLU_WORK_REMOTE ??
...
async function runStart(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(staticDir)) {
    console.error(`[zhanlu-work-web] static dir not found: ${staticDir}`);
    console.error(`  hint: pass --static-dir <path> pointing to the SPA build output`);
    process.exit(1);
  }

  console.log(`[zhanlu-work-web] version    : ${version}`);
  console.log(`[zhanlu-work-web] data dir   : ${dataDir}`);
  console.log(`[zhanlu-work-web] log dir    : ${logDir}`);
  console.log(`[zhanlu-work-web] static dir : ${staticDir}`);
  console.log(`[zhanlu-work-web] backend bin: ${backendBin}`);
  console.log(`[zhanlu-work-web] launching  : port=${port} allowRemote=${allowRemote}`);
  ...
  if (!backendAvailable) {
    ...
    console.warn('   To enable backend: download aioncore and set ZHANLU_WORK_BACKEND_BIN.');
    console.warn('');
    ...
    console.log('');
    console.log('Zhanlu Work WebUI (frontend only) is ready');
    console.log(`  Local  : ${handle.localUrl}`);
    if (handle.networkUrl) console.log(`  Network: ${handle.networkUrl}`);
    if (autoOpenBrowser) {
      const openResult = openBrowserUrl(handle.localUrl);
      if (openResult.ok) {
        console.log(`[zhanlu-work-web] opened ${handle.localUrl} in your browser.`);
      } else {
        console.warn(`[zhanlu-work-web] could not open the browser automatically: ${openResult.reason}`);
      }
    }
    console.log('');
  ...
  const shutdown = async (signal: string): Promise<void> => {
    ...
    shuttingDown = true;
    console.log(`\n[zhanlu-work-web] received ${signal}, stopping...`);
    try {
      if (currentHandle) await currentHandle.stop();
    } catch (err) {
      console.error('[zhanlu-work-web] stop failed:', err);
    }
    process.exit(0);
  };
...
/**
 * `zhanlu-work-web resetpass` — spin up the backend just long enough to POST
...
async function runResetPassword(flags: Map<string, string | true>): Promise<void> {
  ...
  if (!fs.existsSync(backendBin)) {
    console.error(`[zhanlu-work-web] backend binary not found: ${backendBin}`);
    console.error('  hint: pass --backend-bin <path> or set ZHANLU_WORK_BACKEND_BIN');
    process.exit(1);
  }
  const dataDir = resolveDataDir(flags);
  fs.mkdirSync(dataDir, { recursive: true });
  ...
  console.log(`[zhanlu-work-web] resetting admin password in ${dataDir}`);
  ...
  try {
    ...
    if (!ready) {
      console.error('[zhanlu-work-web] backend did not become ready within 15s');
      process.exit(1);
    }
    ...
    if (!res.ok) {
      console.error(`[zhanlu-work-web] /api/webui/reset-password returned ${res.status}`);
      process.exit(1);
    }
    ...
    if (!newPassword) {
      console.error('[zhanlu-work-web] reset-password response missing new_password');
      process.exit(1);
    }
    console.log(`[zhanlu-work-web] username: ${username}`);
    console.log(`[zhanlu-work-web] new password: ${newPassword}`);
    console.log('[zhanlu-work-web] existing sessions have been invalidated.');
  } finally {
    try {
      await handle.stop();
    } catch {
...
async function main(): Promise<void> {
  ...
  if (command === '--help' || command === 'help' || command === '-h') {
    console.log(`Usage: zhanlu-work-web <command> [options]
...
Options for start:
  ...
  --data-dir <path>       Override data dir (default: ~/.zhanlu-work-web)
...
Options for resetpass:
  --data-dir <path>       Which data dir to reset (default: ~/.zhanlu-work-web)
  --backend-bin <path>    Override backend binary path

Environment variables:
  ZHANLU_WORK_PORT, ZHANLU_WORK_ALLOW_REMOTE, ZHANLU_WORK_DATA_DIR,
  ZHANLU_WORK_LOG_DIR, ZHANLU_WORK_BACKEND_BIN, ZHANLU_WORK_OPEN_BROWSER
...
`);
  ...
  if (command !== 'start') {
    ...
    console.error('Usage: zhanlu-work-web [start|resetpass|version|help]');
    process.exit(1);
  }
...
main().catch((err: Error) => {
  console.error('[zhanlu-work-web] fatal:', err.message);
