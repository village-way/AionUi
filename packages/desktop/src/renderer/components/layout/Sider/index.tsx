...
import { SiderToolbar, SiderSearchEntry, SiderScheduledEntry, SiderCapabilitiesEntry } from './SiderNav';
...
const Sider: React.FC<SiderProps> = ({ onSessionClick, collapsed = false }) => {
  ...
  const handleCapabilitiesClick = () => {
    ...
    closePreview();
    setIsBatchMode(false);
    Promise.resolve(navigate('/capabilities')).catch((error) => {
      console.error('Navigation failed:', error);
    });
    if (onSessionClick) {
      onSessionClick();
        ...
        {isSettings ? (
          ...
          <div className='size-full flex flex-col gap-2px'>
            ...
            <SiderCapabilitiesEntry
              isMobile={isMobile}
              isActive={pathname.startsWith('/capabilities')}
              collapsed={collapsed}
              siderTooltipProps={siderTooltipProps}
