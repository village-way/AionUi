...
const APP_USER_MODEL_ID = 'Ecloud.ZhanluWork';
...
const isE2ETestMode = process.env.ZHANLU_WORK_E2E_TEST === '1' || process.env.AIONUI_E2E_TEST === '1';
const skipSingleInstanceLock =
  isE2ETestMode || process.env.ZHANLU_WORK_MULTI_INSTANCE === '1' || process.env.AIONUI_MULTI_INSTANCE === '1';
const deepLinkFromArgv = process.argv.find((arg) => arg.startsWith(`${PROTOCOL_SCHEME}://`));
const gotTheLock = skipSingleInstanceLock ? true : app.requestSingleInstanceLock({ deepLinkUrl: deepLinkFromArgv });
if (!gotTheLock) {
  console.warn('[ZhanluWork] Another instance is already running; current process will exit.');
  app.quit();
} else {
  app.on('second-instance', (_event, argv, _workingDirectory, additionalData) => {
    ...
    if (app.isReady()) {
      showOrCreateMainWindow({
        mainWindow,
        createWindow: () => {
          console.log('[ZhanluWork] second-instance received with no active main window, recreating main window');
          createWindow();
        },
      });
    }
...
function registerCronResumeBridge(backendPort: number): void {
  ...
  const onResume = () => {
    void fetch(`http://127.0.0.1:${backendPort}/api/cron/internal/system-resume`, {
      ...
      console.error('[ZhanluWork] Failed to notify backend about system resume:', error);
    });
  };
...
const scheduleBackendMigrations = (): void => {
  ...
  void (async () => {
    try {
      ...
      console.info('[ZhanluWork] runBackendMigrations completed');
    } catch (error) {
      console.error('[ZhanluWork] Backend migration hook threw:', error);
    }
  })();
};
...
function markBackendReady(backendPort: number, source: string): void {
  ...
  console.log(`[ZhanluWork] ${source} ready (port=${backendPort})`);
...
function resolveDebugBackendStartupFailure(): BackendStartupFailureInfo | null {
  ...
  if ((app.isPackaged && !isE2ETestMode) || isWebUIMode || isResetPasswordMode) {
    console.warn('[ZhanluWork] Ignoring AIONUI_DEBUG_BACKEND_STARTUP_FAILURE outside desktop dev/e2e mode.');
    return null;
  }
  ...
  console.warn(`[ZhanluWork] Ignoring unknown AIONUI_DEBUG_BACKEND_STARTUP_FAILURE value: ${reason}`);
  return null;
}
...
const createWindow = ({ showOnReady = true }: { showOnReady?: boolean } = {}): void => {
  console.log('[ZhanluWork] Creating main window...');
  ...
  console.log(`[ZhanluWork] Main window created (id=${mainWindow.id})`);
  ...
  if (showOnReady) {
    const showWindow = () => {
      if (!mainWindow.isDestroyed() && !mainWindow.isVisible()) {
        console.log('[ZhanluWork] Showing main window');
        mainWindow.show();
        mainWindow.focus();
      }
    };
    mainWindow.once('ready-to-show', () => {
      console.log('[ZhanluWork] Window ready-to-show');
      showWindow();
    });
    // Belt-and-suspenders: also show on did-finish-load in case ready-to-show already fired
    mainWindow.webContents.once('did-finish-load', () => {
      console.log('[ZhanluWork] Renderer did-finish-load');
      showWindow();
      scheduleBackendMigrations();
    });
  ...
  const disableAutoUpdater =
    process.env.ZHANLU_WORK_DISABLE_AUTO_UPDATE === '1' ||
  ...
  if (!disableAutoUpdater) {
    ...
    console.log('[ZhanluWork] Auto-updater disabled via env/CI guard');
  }
  ...
  if (!app.isPackaged && rendererUrl) {
    console.log(`[ZhanluWork] Loading renderer URL: ${rendererUrl}`);
    mainWindow.loadURL(rendererUrl).catch((error) => {
      console.error('[ZhanluWork] loadURL failed, falling back to file:', error.message || error);
      mainWindow.loadFile(fallbackFile).catch((e2) => {
        console.error('[ZhanluWork] loadFile fallback also failed:', e2.message || e2);
      });
    });
  } else {
    console.log(`[ZhanluWork] Loading renderer file: ${fallbackFile}`);
    mainWindow.loadFile(fallbackFile).catch((error) => {
      console.error('[ZhanluWork] loadFile failed:', error.message || error);
    });
  }

  mainWindow.webContents.on('did-fail-load', (_event, errorCode, errorDescription, validatedURL, isMainFrame) => {
    console.error('[ZhanluWork] did-fail-load:', { errorCode, errorDescription, validatedURL, isMainFrame });
  });

  mainWindow.webContents.on('render-process-gone', (_event, details) => {
    console.error('[ZhanluWork] render-process-gone:', details);
    ...
    if (!mainWindow.isDestroyed()) {
      console.log('[ZhanluWork] Attempting to recover from renderer crash by reloading...');

      if (!app.isPackaged && rendererUrl) {
        mainWindow.loadURL(rendererUrl).catch((error) => {
          console.error('[ZhanluWork] Recovery loadURL failed:', error.message || error);
        });
      } else {
        mainWindow.loadFile(fallbackFile).catch((error) => {
          console.error('[ZhanluWork] Recovery loadFile failed:', error.message || error);
        });
      }
    }
  });
  ...
  mainWindow.webContents.on('unresponsive', () => {
    console.warn('[ZhanluWork] Renderer became unresponsive');
  });

  mainWindow.on('closed', () => {
    console.log('[ZhanluWork] Main window closed');
  });
...
const handleAppReady = async (): Promise<void> => {
  ...
  const mark = (label: string) => console.log(`[ZhanluWork:ready] ${label} +${Math.round(performance.now() - t0)}ms`);
  mark('start');
  ...
  try {
    ...
    console.error('[ZhanluWork] Failed to restore zoom factor:', error);
  ...
  try {
    ...
    console.error('[ZhanluWork] Failed to restore window bounds:', error);
  ...
  if (isResetPasswordMode) {
    ...
    try {
      // Inside Electron (`ZhanluWork --webui` or packaged `zhanlu-work-web` mode that
...
// Register zhanlu-work:// as the default protocol client
...
// macOS: handle zhanlu-work:// URLs via the open-url event
...
void app
  ...
  .catch((error) => {
    ...
    console.error('[ZhanluWork] App initialization failed:', error);
    app.quit();
  });
...
app.on('will-quit', () => {
  console.log('[ZhanluWork] will-quit — all cleanup should be complete');
});

app.on('quit', (_event, exitCode) => {
  console.log(`[ZhanluWork] quit (exitCode=${exitCode})`);
});
