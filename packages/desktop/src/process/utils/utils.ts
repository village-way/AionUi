...
export const getTempPath = () => {
  ...
  return path.join(rootPath, 'zhanlu-work');
};
 ...
 * Release builds use ~/.zhanlu-work; dev builds use ~/.zhanlu-work-dev.
 * 获取数据目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.zhanlu-work，Dev 模式使用 ~/.zhanlu-work-dev。
 */
export const getDataPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const dataPath = path.join(rootPath, 'zhanlu-work');
  return ensureCliSafeSymlink(dataPath, getEnvAwareName('.zhanlu-work'));
};

/**
 * Get config path, using CLI-safe symlink on macOS.
 * Release builds use ~/.zhanlu-work-config; dev builds use ~/.zhanlu-work-config-dev.
 * 获取配置目录路径，macOS 上使用符号链接。
 * Release 使用 ~/.zhanlu-work-config，Dev 模式使用 ~/.zhanlu-work-config-dev。
 */
export const getConfigPath = (): string => {
  const rootPath = getElectronPathOrFallback('userData');
  const configPath = path.join(rootPath, 'config');
  return ensureCliSafeSymlink(configPath, getEnvAwareName('.zhanlu-work-config'));
};
...
export async function verifyDirectoryFiles(dir1: string, dir2: string): Promise<boolean> {
  try {
    ...
    console.warn('[ZhanluWork] Error verifying directory files:', error);
    return false;
  }
}
...
export const copyFilesToDirectory = async (
  ...
  for (const file of files) {
    ...
    try {
      ...
      console.warn(`[ZhanluWork] Source file does not exist, skipping: ${absoluteFilePath}`);
      console.warn(`[ZhanluWork] Original path: ${file}`);
      // 跳过不存在的文件，而不是抛出错误
      continue;
    }
    ...
    try {
      ...
      console.error(`[ZhanluWork] Failed to copy file from ${absoluteFilePath} to ${destPath}:`, error);
      // 继续处理其他文件，而不是完全失败
    }
